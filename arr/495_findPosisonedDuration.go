func findPoisonedDuration(timeSeries []int, duration int) int {
    ...
    for i:=range timeSeries {
        var tmp []int
        tmp=append(tmp,timeSeries[i])
        tmp=append(tmp,timeSeries[i]+duration)
        intervals=append(intervals,tmp)
    }
    sort.Sort(Interval(intervals))
    resMerge:=merge(intervals)
    fmt.Println(intervals,resMerge)
    for i:=range resMerge {
        res+=(resMerge[i][1]-resMerge[i][0])
    }
    return res
}
