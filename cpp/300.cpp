...
设 j∈[0,i)，考虑每轮计算新 dp[i]时，遍历 [0,i)列表区间，做以下判断： 
    ...
    上述所有 情况 下计算出的 dp[j]+1的最大值，为直到 i的最长上升子序列长度（即 dp[i]）。实现方式为遍历 j 时，每轮执行 dp[i]=max(dp[i],dp[j]+1)。 转移方程： dp[i]=max(dp[i],dp[j]+1)for j in [0,i)
...
public:
    int lengthOfLIS(vector<int>& nums) {
        ...
        for(int i=1;i<nums.size();i++){
            // 遍历 j，每轮执行 dp[i]=max(dp[i],dp[j]+1) 
